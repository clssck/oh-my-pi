    ...
    *,
    ...
    try:
        ...
        if settings is not None and settings.ack_dynamic:
            text = await _generate_ack_text(settings, title=title, author=directive.author, request=directive.body)
            if text:
                body = persona.directive_ack_dynamic(text=text, comment_id=comment.id)
        if body is None:
            body = persona.directive_ack_comment(author=directive.author, request=directive.body, comment_id=comment.id)
...
async def handle_comment(
    *,
    ...
    if directive is not None:
        await _ack_directive(
            github, repo.full_name, issue.number, comment, directive, settings=settings, title=issue.title
        )
