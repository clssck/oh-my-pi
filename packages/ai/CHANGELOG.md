...
- Stopped treating OpenAI Codex WebSocket connect-timeout as a session-fatal error. When the Codex WS handshake hit the 10s connect cap (most often because startup event-loop contention delayed the `open` event, not because the network was actually broken), the error was classified in `CODEX_WEBSOCKET_FATAL_PATTERNS` and permanently set `state.disableWebsocket = true`, exiling the entire session to SSE. SSE is functional but disables the `previous_response_id` append-chain optimization, so every continuation turn re-uploaded the full prior context (observed ~970 fresh tokens per continuation vs ~110 on WS for the same workload). Connect-timeout now soft-falls back to SSE for the offending request and leaves WS enabled for the next request. Fatal classification is retained for protocol-level failures (`websocket error:`, `websocket closed before open`). Measured live on `openai-codex/gpt-5.4` with `PI_CODEX_WEBSOCKET=1`: continuation-turn input dropped from 971 to 112 tokens (~88% reduction).
## [14.1.3] - 2026-04-17
