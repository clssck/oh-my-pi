...
export class SessionObserverOverlayComponent extends Container {
	...
	#renderToolCallLines(
		...
		if (argSummary) {
			const maxWidth = Math.max(40, (process.stdout.columns || 80) - INDENT.length - 2);
			for (const line of argSummary.split("\n")) {
				lines.push(`${INDENT}${theme.fg("dim", truncateToWidth(replaceTabs(line), maxWidth))}`);
			}
		}
	...
	#handleViewerInput(keyData: string): void {
		...
		if (matchesKey(keyData, "pageDown")) {
			...
			this.#rebuildAndScroll(preserveScroll);
			return;
		}
	...
	/** Rebuild transcript lines (which depend on selectedEntryIndex/expandedEntries) and optionally scroll to selection */
	#rebuildAndScroll(preserveScroll = false): void {
