...
export class DaxnutsComponent implements Component {
	...
	render(width: number): string[] {
		...
		if (textPhase > 0 || this.tick >= this.maxTicks) {
			lines.push(center(chalk.cyan("Free Kimi K2.5 via OpenCode Zen")));
			lines.push(center(chalk.green('"Powered by daxnuts"')));
			lines.push(center(chalk.gray("— @thdxr")));
		} else {
			lines.push("");
			lines.push("");
			lines.push("");
		...
		if (textPhase > 2 || this.tick >= this.maxTicks) {
			lines.push(center(chalk.dim("Try OpenCode")));
			lines.push(center(chalk.blue("https://mistral.ai/news/mistral-vibe-2-0")));
		} else {
			lines.push("");
			lines.push("");
		}
